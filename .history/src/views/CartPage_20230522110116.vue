<template>
  <div>
    <h1>Shopping Cart</h1>
    <div class="product" v-for="product in this.$store.state.lineItems" :key="product.metadata.id">
      <div class="cart-item">
        <img :src="product.metadata.image">
        <div class="item-details">
          <div class="name">{{ product.metadata.productName }}</div>
          <div class="quantity">Quantity: {{ product.quantity }}</div>
        </div>
        <div class="price">Price: {{ Number(product.metadata.cost*product.quantity) }}</div>
      </div>
    </div>
