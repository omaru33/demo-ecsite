    ...
    <div class="product" v-for="product in this.$store.state.cartItems" :key="product.id">
      <div class="cart-item">
        ...
        <div class="item-details">
          ...
          <div class="cost">Price: {{ product.cost }}</div>
          <div class="quantity">Quantity: {{ product.quantity }}</div>
        </div>
        <div class="price">Total: {{ Number(product.cost*product.quantity) }}</div>
      </div>
    </div>
    {{ allTotal }}
...
export default {
  ...
  computed:{
    allTotal(){
      return this.$store.state.cartItems.reduce((total, item) => total + (Number(item.cost) * Number(item.quantity)), 0);
    }
  }
};
</script>
