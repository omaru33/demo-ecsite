...
export default createStore({
    ...
    return{
      ...
      products: [
        ...
        },
        {
          ...
          description:"清潔t感あふれる白いワンピース。柔らかな素材で、軽やかな着心地が魅力的です。レースのディテールが施されたエレガントなデザインで、女性らしさを引き立てます。袖は三分丈で、腕を華奢に見せてくれます。ウエストにはベルトが付属しており、ウエストラインを強調したシルエットが美しく仕上がっています。ヒールと合わせて、結婚式やパーティーなどのフォーマルなシーンにぴったりのアイテムです。また、サンダルやスニーカーと合わせて、デイリーカジュアルな着こなしも楽しめます。オールシーズン着用可能で、長く愛用できるアイテムです。",
          cost:"6600",
  ...
  mutations: {
    addcartItem(state, cartItem) {
      const existingItem = state.cartItems.find(item => item.price === cartItem.price);
      if (existingItem) {
        existingItem.quantity+=state.selectedQuantity;
        state.lineItems.find(item => item.price === existingItem.price).quantity = existingItem.quantity;
      } else {
        const lineItem = {
